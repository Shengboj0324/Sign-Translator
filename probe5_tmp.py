...
def evaluate(rec,head,fe):
    feats,tg=batch(fe,noise=0.03,seed=900)
    rec.eval();head.eval()
    with torch.no_grad():
...
import statistics
for bw,brw,name in [(0.0,0.0,'ASR only'),(0.5,0.3,'full')]:
    f1s=[];eces=[]
    for seed in (0,1,2):
        rec,head,obj,fe=train(bw,brw,seed=seed,iters=220)
        f1,ece=evaluate(rec,head,fe); f1s.append(f1); eces.append(ece)
    print(f'{name:10s} F1 {statistics.mean(f1s):.3f} (each {[round(x,3) for x in f1s]})  '
          f'ECE {statistics.mean(eces):.4f} (each {[round(x,4) for x in eces]})')
