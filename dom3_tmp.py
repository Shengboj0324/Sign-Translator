...
src_tr,y_tr,plans_tr=dataset(240,0); src_ev,y_ev,plans_ev=dataset(90,999)
fs=[];js=[]
for trial in range(3):
    torch.manual_seed(trial)
    enc=EvidenceEncoder(K+2,d_model=32,num_layers=1); head=ContentHead(32,K); dec=HeavyDecoder(V.size,d_model=32,num_layers=4)
    factorized_train(enc,head,dec,src_tr,y_tr,plans_tr,stage_a_steps=120,stage_b_steps=120)
    fs.append(representation_probe_accuracy(enc,src_tr,y_tr,src_ev,y_ev,epochs=150))
    torch.manual_seed(trial)
    enc2=EvidenceEncoder(K+2,d_model=32,num_layers=1); dec2=HeavyDecoder(V.size,d_model=32,num_layers=4)
    joint_train(enc2,dec2,src_tr,plans_tr,steps=240)
    js.append(representation_probe_accuracy(enc2,src_tr,y_tr,src_ev,y_ev,epochs=150))
